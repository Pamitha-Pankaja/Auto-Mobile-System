...
//         owner: { id: user.id }
//       },
//       technician: { id: 3 },  // Replace with the actual technician ID
...
//                 <button type="submit" className="btn btn-primary" disabled={!selectedService || !selectedDate || !selectedavailableTimeSlots}>
//                   Book Appointment
//                 </button>
//               </form>
//             </div>
...
const Appointment = () => {
  ...
  const fetchAvailableTimeSlots = async (selectedDate) => {
    try {
      const url = `${basurl}/api/appointments/available-slots?date=${selectedDate}&serviceId=${selectedService.id}`;
      const response = await axios.get(url);
      setAvailableTimeSlots(response.data);
    } catch (error) {
      console.error('Error fetching time slots:', error);
